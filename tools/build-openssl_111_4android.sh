...
BUILD_DIR=$PWDPATH/../output/android_build
OUT_DIR=$PWDPATH/../output/android
...
patch -b -p1 -N -d "openssl-${OPENSSL_VERSION}" < "${PWDPATH}/openssl-1.1.1c.patch"
...
build_the_thing() {
    ...
    make install_ssldirs DESTDIR=$DESTDIR || exit 128   
}
...
do
    ...
    mkdir -p $OUT_DIR/openssl-$build_target/include
    cp -R $DESTDIR/usr/local/include/* $OUT_DIR/openssl-$build_target/include
    mkdir -p $OUT_DIR/openssl-$build_target/lib
    cp -R $DESTDIR/usr/local/lib/*.so $OUT_DIR/openssl-$build_target/lib
    cp -R $DESTDIR/usr/local/lib/*.a $OUT_DIR/openssl-$build_target/lib
done
